/**
 * Config-dump entry for `kibborg --dump-config`: compose the profile's patch
 * layers through the include plugin's patch algorithm without booting and
 * without evaluating `!!js`, with one source layer per bundle, the profile's
 * own patch file, and each `--patch` overlay.
 * @module @kibborg/cli/dump-config
 */
...
/** Diagnostic program name every message of this module uses. */
const NAME = 'kibborg'

/**
 * Print the composed profile tree with comments naming each source file and
 * patch layer.
 ...
 */
export function runDumpConfig(defaultOnly: boolean, patches: readonly string[]): void {
  const loaded = prepareProfile(!defaultOnly)
