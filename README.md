...
| **Полный вывод инструментов** | Для каждого вызова: `IN` с аргументами как их прислала модель, `OUT` с полным ответом инструмента, diff со знаками `+`/`−` и сводкой `+N −M`, время выполнения. Ничего не обрезается по ширине. |
...
| **Сворачивание** | Блок длиннее 20 строк показывается началом и строкой «⋯ ещё N строк — клик, чтобы развернуть». |
...
- длинные строки **переносятся**, а не обрезаются; блок целиком больше 20 строк сворачивается строкой «⋯ ещё N строк — клик, чтобы развернуть».
