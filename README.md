...
- глифы действий: `◐` идёт работа, `✔️` успех, `✕` ошибка, `▣` файл, `→` делегирование, `◌` размышление;
...
- длинный аргумент обрезается по колонкам, которые остались после глифа и хвоста: у пути сохраняются начало и имя файла (`Читает C:/Users/lex66/…/dist/index.js`), у команды — начало, а счётчики `+N`, `−M` и время видны всегда;
...
- `◆` — сессия, в которую вы печатаете; `┌─ ◐` и `└─ ✔️` — рамка ветки субагента: она открывается, когда агент начинает, и закрывается, когда он закончил;
...
| Повреждённый отчёт терминала | записи `scope: input`, `action: noise`: хвосты вида `[13;28;13;1;0;1_`, которые терминал прислал без ведущего `ESC`, отброшены вместо попадания в поле ввода |
...
| В поле ввода попадает мусор вида `[13;28;13;1;0;1_` | обновите CLI: такие хвосты отбрасываются и пишутся в журнал как `scope: input`; обойти целиком — `KIBBORG_NO_WIN32_INPUT=1` |
