...
| | |
|---|---|
| **Живой TUI** | Кадр собирается в буфер ячеек и печатается разницей. Верхняя строка отвечает «где я» (`≡ ветка  каталог … ctx 18%`), поле ввода — рамка, в нижнюю границу которой вписаны модель и режим (`╰─ kibborg/… · Agent ───── 3 агента · 7 задач ─╯`), а строка подсказок клавиш между ними меняется по контексту. |
...
| **Проверяемость** | 272 юнит-теста, `oxlint` без замечаний, PTY-сценарии меню/команд/полноэкранного режима и инструмент кадров `tests/frame-render.mjs`. |
...
| колесо мыши | прокрутка ленты (терминальная: Kibborg мышь не перехватывает) |
