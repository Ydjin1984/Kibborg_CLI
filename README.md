...
| `Shift+Enter`, `Ctrl+J` | перевод строки в строке ввода |
| `Esc` | закрыть список, прервать ход, очистить ввод (никогда не выходит) |
| `Ctrl+C` | прервать ход; на пустом вводе — выйти (второе нажатие) |
| `Ctrl+D` | выйти |
| `Ctrl+O` | открыть живую панель данных; повторно — закрыть |
| `Tab` / `Shift+Tab` | переключение вкладок панели; в списке — вниз/вверх по пунктам |
| `↑` / `↓` | прокрутка ленты на строку (на пустом вводе) либо история ввода |
...
| клик по «⋯ ещё N строк» | развернуть или свернуть длинный блок |
...
## Лента: инструменты, diff, Markdown

Каждый вызов инструмента показывается целиком:

```
    ⚙  edit   src/build.ts   +2 −1   135ms
      IN  {
...
- длинные строки **переносятся**, а не обрезаются; блок целиком больше 5 строк сворачивается строкой «⋯ ещё N строк — клик, чтобы развернуть».

Ответы модели рендерятся как Markdown: заголовки и первое заключение — жирные, списки — с маркерами и отступами, таблицы — с выравниванием колонок и линией под шапкой, код — с меткой языка и вертикальным рельсом, ссылки — акцентные и подчёркнутые.

---

## Кто работает: агенты, модели и роли

Один ход не всегда делает одна модель. В режиме оркестратора беседу ведёт «голова» (она планирует и делегирует), а тяжёлую работу выполняет локальный исполнитель — это отдельный субагент со своей сессией. Лента подписывает строки агентом, поэтому видно, какая модель что сделала:

```
  ◆  KIBORG   deepseek-official/deepseek-v4-flash   ORCHESTRATOR
    ⚙  executor   посчитать строки README
  ├─ ◇  посчитать строки README   kibborg/Kibborg_Flash_v5.7   EXECUTOR
  │      ✓  bash   (Get-Content README.md).Count
  │      ✓  grep   обработать текст
  └─ ✓  посчитать строки README
  ◆  KIBORG   deepseek-official/deepseek-v4-flash   ORCHESTRATOR
    ⚙  edit   README.md   +12 −3
```

- `◆` — сессия, в которую вы печатаете; `◇` — субагент; `✓` — ветка закрыта;
...
- цвет агента выводится из его имени, поэтому разные модели в одном прогоне видно сразу;
- строки субагента сдвинуты вправо (`│`), его собственный текст в ленту не попадает — видны только вызовы инструментов: чем он занят, а не его переписка.
