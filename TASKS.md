...
| 1 | todo | Каркас экрана: верхняя строка, лента без рамок с временем, строка работы, рамка ввода со статусом, подсказки клавиш, скроллбар |
...
| 4 | todo | Клавиши Grok: `Tab`, `↑↓`, `h`/`l`, `y`, `Ctrl+C` отменяет ход, `Esc` — нет, `Shift+Tab`, `Ctrl+x` |
