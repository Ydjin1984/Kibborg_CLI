...
| 2 | **в работе** | Отображение работы: человеческие действия (`Запускает …`, `Читает …`, `Делегирует: …`) и строка работы сделаны; 2.2 сворачивание и раскрытие клавишей сделаны; остались 2.3 diff в стиле Grok, 2.4 блоки размышлений, 2.5 субагенты как дочерние сессии |
| 3 | todo | Цвета: нейтральная база, приглушённые границы, один акцент, семантика из ANSI, без «радуги» на агентов |
| 4 | **частично** | Клавиши Grok: `Ctrl+C` отменяет ход, `Esc` — нет, `↑↓` выбирают запись, `h`/`l` сворачивают и разворачивают, `y` копирует сделаны; остались `Tab`-фокус, `Shift+Tab`, `Ctrl+x` |
