...
Сделано: `log.ts` конденсирует записи длиннее `COLLAPSED_LINES` (порог входа — 20 строк; текущее значение 5 введено переработкой ленты по образцу Grok CLI) — ответ модели, вывод инструмента и diff. Показывается начало и строка `⋯ ещё N строк — Enter, чтобы развернуть`; она несёт идентификатор записи и признак `collapsed`, поэтому `app.ts` сопоставляет строку кадра с записью и по выбранной записи переключает `LogEntry.expanded` (повторное нажатие сворачивает). Мышь по умолчанию не перехватывается, поэтому раскрытие — клавиатурное; при `KIBBORG_MOUSE=1` работает и клик по строке.

Проверка: юнит-тест поверхности — длинная запись сжимается до порога с маркером, полный блок скрыт, после `expanded: true` видны все строки; живой PTY-прогон с ответом на 30 строк показал маркер «ещё N строк — Enter, чтобы развернуть».
...
| 5 | **done** | Тесты (286), PTY-кадры (frame, menu, commands, fullscreen), README/CHANGELOG/TASKS/план по факту. Остаток зафиксирован в `UI_GROK_PLAN.md`: `--fullscreen` и `KIBBORG_INLINE=1` отстают от основной поверхности по выбору записи и строке работы |
...
### Обрезка строки инструмента и мусор во вставке

Пользователь сообщил о двух вещах: длинный вывод инструмента не читается (обрезан целиком), а при вставке текста и скопированного промпта в поле ввода попадает мусор вида `[13;28;13;1;0;1_`.

Разобрано:

- **Заголовок строки инструмента.** `toolTitle` знал только бюджет символов (160, у части инструментов 80/88), а поверхность обрезала заголовок по этому бюджету, а не по колонкам кадра, поэтому длинный путь терял конец, а вместе с ним — имя файла, то есть самое узнаваемое. Теперь `clipPath` (в `width.ts`) режет текст по колонкам, оставляя начало и имя файла с расширением, а строка инструмента считает бюджет от того, что осталось после глифа и хвоста (`+N`, `−M`, время, `▸ аргументы · вывод`): на 114 колонках видно `Читает C:/Users/lex66/AppData/Roaming/…/dist/index.js   1.2s`. Тот же помощник применён в истории `/resume`, где аргумент обрезался `slice(0, 80)` и терял имя файла.
- **Мусор во вставке.** Windows-терминал в режиме win32-input-mode присылает отчёт на каждую клавишу, а вставка — пачка отчётов; когда граница чтения попадала между `ESC` и остальным отчётом, префикс разрешался в отдельный `Escape` (таймер `escape-idle`), а хвост `[13;28;13;1;0;1_` печатался в черновик. Причина подтверждена issue microsoft/terminal#17656 («win32-input-mode: clipboard paste weird escape sequences») и правкой upstream «Fix parsing of chunked win32-input-mode sequences». Хвосты теперь распознаются по форме (`orphanTail` в `input.ts`: win32-отчёт, SGR-мышь, завершение bracketed paste, CSI-клавиша), отбрасываются и записываются в журнал как `scope: input, action: noise`; символ вставки из win32-отчёта восстанавливается, а отчёт управляющей клавиши — нет, потому что действовать на непожатую клавишу хуже потери. Распознаются только хвосты, декодируемые в клавишу, поэтому текст вроде `[0;30m` и `[1;2]` доходит до поля ввода как есть.
- **Диагностика.** `EscapeIdle.push` теперь возвращает `{keys, noise}`, и обе поверхности (`repl`, `fullscreen`) пишут шум в журнал: следующее повторение видно в `/logs` без воспроизведения сессии.

Проверка: 309 тестов (`packages/tui/tests/tool-row.spec.ts` — заголовок влезает в 114/80/60 колонок, содержит `…`, оканчивается `index.js` и сохраняет `+12 −3`; `input.spec.ts` — хвосты win32/CSI/paste/мыши отбрасываются, символ вставки восстанавливается, `[0;30m` печатается как текст; `escape-idle.spec.ts` — хвост после разошедшегося `Escape` доходит как шум), `oxlint` чистый. Живой PTY-прогон (`frame-render.mjs`): после `keys:[13;28;13;1;0;1_hello` в поле ввода стоит `> hello`, а журнал получает `{"scope":"input","action":"noise","details":{"fragments":["orphan-win32:[13;28;13;1;0;1_]"]}}`; после `keys:[0;30m green` в поле ввода стоит `> [0;30m green`.