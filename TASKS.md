...
| 1 | **в работе** | Каркас экрана: 1.1 верхняя строка (`≡ branch cwd … ctx N% [Panel]`), 1.2 лента без рамок со временем сообщений, 1.4 рамка ввода со статусом в нижней границе, 1.5 контекстная строка клавиш — сделаны; остались 1.3 строка работы, 1.6 скроллбар |
| 2 | todo | Отображение работы: `♦ Run/Read/Edit/Grep/Ask/Thought`, сворачиваемые результаты, diff, размышления, субагенты |
| 3 | todo | Цвета: нейтральная база, приглушённые границы, один акцент, семантика из ANSI, без «радуги» на агентов |
| 4 | **частично** | Клавиши Grok: 4.2 сделан (`Ctrl+C` отменяет ход, `Esc` — нет, вместо этого читает ленту и говорит, чем прервать); остались `Tab`, `↑↓`, `h`/`l`, `y`, `Shift+Tab`, `Ctrl+x` |
